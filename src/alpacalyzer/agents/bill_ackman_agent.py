...
def bill_ackman_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        ackman_output = generate_ackman_output(ticker=ticker, analysis_data=analysis_data)

        if ackman_output is None:
            progress.update_status("bill_ackman_agent", ticker, "Failed: No output from LLM")
            # Still create an entry with neutral sentiment when no sentiment data
            ackman_analysis[ticker] = {
                "signal": "neutral",
                "confidence": 0,
                "reasoning": "Bill Ackman analysis failed or returned no data",
            }
            continue
...
def analyze_business_quality(metrics: list[Any], financial_line_items: list[Any]) -> dict[str, Any]:
    """
...
def analyze_financial_discipline(metrics: list[Any], financial_line_items: list[Any]) -> dict[str, Any]:
    """
...
def analyze_activism_potential(financial_line_items: list[Any]) -> dict[str, Any]:
    """
    Bill Ackman often engages in activism if a company has a decent brand or moat but is underperforming operationally.
...
def analyze_valuation(financial_line_items: list[Any], market_cap: float) -> dict[str, Any]:
    """
...
def generate_ackman_output(
    ...
    human_template = """Based on the following data, create an Ackman-style investment signal.
