...
def cathie_wood_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        if cw_output is None:
            progress.update_status("cathie_wood_agent", ticker, "Failed: No output from LLM")
            # Still create an entry with neutral sentiment when no sentiment data
            cw_analysis[ticker] = {
                "signal": "neutral",
                "confidence": 0,
                "reasoning": "Cathie Wood analysis failed or returned no data",
            }
            continue
...
def analyze_disruptive_potential(metrics: list[Any], financial_line_items: list[LineItem]) -> dict[str, Any]:
    """
...
def analyze_innovation_growth(metrics: list[Any], financial_line_items: list[LineItem]) -> dict[str, Any]:
    """
...
def analyze_cathie_wood_valuation(financial_line_items: list[LineItem], market_cap: float) -> dict[str, Any]:
    """
    Cathie Wood often focuses on long-term exponential growth potential.

    We can do a simplified approach looking for a large total addressable market (TAM) and the
