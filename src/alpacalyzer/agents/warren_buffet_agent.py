...
def warren_buffett_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        total_score = float(fundamental_analysis["score"]) + float(consistency_analysis["score"])
        ...
        if buffett_output is None:
            progress.update_status("warren_buffett_agent", ticker, "Failed: No output from LLM")
            # Still create an entry with neutral sentiment when no sentiment data
            buffett_analysis[ticker] = {
                "signal": "neutral",
                "confidence": 0,
                "reasoning": "Warren Buffet analysis failed or returned no data",
            }
            continue
...
def generate_buffett_output(
    ...
    human_template = """Based on the following data, create the investment signal as Warren Buffett would.
