...
from alpacalyzer.data.api import (
    get_company_news,
...
def charlie_munger_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        valuation_analysis = calculate_munger_valuation(
            financial_line_items, (market_cap if market_cap is not None else 0.0)
        )
        ...
        munger_output = generate_munger_output(ticker=ticker, analysis_data=analysis_data)

        if munger_output is None:
            progress.update_status("charlie_munger_agent", ticker, "Failed: No output from LLM")
            # Still create an entry with neutral sentiment when no sentiment data
            munger_analysis[ticker] = {
                "signal": "neutral",
                "confidence": 0,
                "reasoning": "Charlie Munger analysis failed or returned no data",
            }
            continue
...
def analyze_moat_strength(metrics: list[Any], financial_line_items: list[Any]) -> dict[str, Any]:
    """
...
def analyze_management_quality(financial_line_items: list[Any], insider_trades: list[Any]) -> dict[str, Any]:
    """
    ...
    if fcf_values and net_income_values and len(fcf_values) == len(net_income_values):
        ...
        fcf_to_ni_ratios = [
            fcf_values[i] / net_income_values[i]
...
def analyze_predictability(financial_line_items: list[Any]) -> dict[str, Any]:
    """
    ...
    if op_income and len(op_income) >= 5:
        ...
        if positive_periods == len(op_income):
            ...
            details.append(
                f"Unpredictable operations: "
                f"Operating income positive in only {positive_periods}/{len(op_income)} periods"
            )
    else:
...
def calculate_munger_valuation(financial_line_items: list[Any], market_cap: float) -> dict[str, Any]:
    """
...
def analyze_news_sentiment(news_items: list[Any]) -> str:
    """
