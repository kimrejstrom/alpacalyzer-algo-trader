...
from alpacalyzer.trading.alpaca_client import get_stock_bars
...
def technical_analyst_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        prices_df = get_stock_bars(symbol=ticker, request_type="day")
...
def calculate_rsi(prices_df: pd.DataFrame, period: int = 14) -> pd.Series[float]:
    delta = prices_df["close"].diff()
    gain = (delta.where(delta.astype(float) > 0, 0)).fillna(0)
    loss = (-delta.where(delta.astype(float) < 0, 0)).fillna(0)
...
def calculate_bollinger_bands(prices_df: pd.DataFrame, window: int = 20) -> tuple[pd.Series[float], pd.Series[float]]:
...
def calculate_ema(df: pd.DataFrame, window: int) -> pd.Series[float]:
    """
...
def calculate_hurst_exponent(price_series: pd.Series[float], max_lag: int = 20) -> float:
    """
    ...
    try:
        ...
        return float(reg[0])  # Hurst exponent is the slope
